...
export default function PokedexScreen() {
  ...
  const getApiUrl = () => {
    const COMPUTER_IP = "100.66.101.40";
    if (Platform.OS === "web") {
  ...
  const renderPokemonCard = ({ item }: { item: Pokemon }) => (
    ...
    >
      <Image
        source={{
          uri: `${API_BASE_URL}/api/images/pokemon/${item.pokemon_id}`,
        }}
      ...
      >
        {selectedPokemon && (
          <ScrollView style={styles.modalContainer}>
            ...
            <Image
              source={{
                uri: `${API_BASE_URL}/api/images/pokemon/${selectedPokemon.pokemon_id}`,
              }}
            ...
            <Text style={styles.modalName}>{selectedPokemon.pokemon_name}</Text>

            <View style={styles.modalTypes}>
              {selectedPokemon.types.map((type, index) => (
                <View
                  key={index}
                  style={[styles.modalTypeBadge, getTypeStyle(type)]}
                >
                    ...
                    <View style={styles.damageTypesContainer}>
                      {damageRelations.takes_double_damage_from.map(
                        (type: string, idx: number) => (
                          <View
                            key={idx}
                            style={[styles.damageTypeBadge, getTypeStyle(type)]}
                          >
